...
public:
	MainWindow(GLFWwindow* window) : window(window), width(0), height(0) {
		...
		auto default_scene = std::make_unique<Scene>(view, projection); // initialize the default scene
	...
	void updateModelMatrices(std::vector<std::pair<Mesh*, glm::mat4>>* updated_matrices);
