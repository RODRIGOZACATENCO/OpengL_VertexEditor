...
class ObjHandler {
    ...
    glm::vec3 parseVec3() {
            glm::vec3 v;
            do{curr_pos++;} while(file_str[curr_pos]==' ');
            v.x=parseNumber();
            do{curr_pos++;} while(file_str[curr_pos]==' ');
            v.y=parseNumber();
            do{curr_pos++;} while(file_str[curr_pos]==' ');
    ...
    float parseNumber(num_type num_type=FLOAT) {
        ...
        curr_pos++;
        bool dot_flag=false;
        while ( curr_pos<file_str.size() && (std::isdigit(file_str[curr_pos])||file_str[curr_pos]=='.' && !dot_flag ) )
        {
        ...
        try {
            while(curr_pos<file_size) {
                ...
                if(curr_char=='f') {
                    ...
                    curr_pos++;
                    do{curr_pos++;} while(file_str[curr_pos]==' ');
                    data.faces.push_back(parseNumber());
                    do{curr_pos++;} while(file_str[curr_pos]==' ');
                     data.faces.push_back(parseNumber());
                    do{curr_pos++;} while(file_str[curr_pos]==' ');
                     data.faces.push_back(parseNumber());
