...
public:
  ...
  unsigned int activeTexture;
  int width, height, nrchannels;
  TextureCubeMap(std::vector<std::string> texturePaths, unsigned int aTexture,
                 bool isTransparent = 0) {
    activeTexture = aTexture;
    glGenTextures(1, &textureID);
    ...
    for (int i = 0; i < texturePaths.size(); i++) {
      unsigned char *data =
          stbi_load(texturePaths[i].c_str(), &width, &height, &nrchannels, 0);
      if (data) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, width,
                     height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        stbi_image_free(data);
      } else {
        std::cout << "Cubemap failed to load at path: " << texturePaths[i]
                  << std::endl;
        stbi_image_free(data);
      }
    }
