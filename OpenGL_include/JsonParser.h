...
public:
  ...
  String& as_string() { return std::get<String>(value); }
  ...
  JsonValue& operator[](const int index) {
    //when acessing array with index
  ...
  struct JsonPrinter {
    ...
    void operator()(const Number n) const { os << std::setprecision(std::numeric_limits<double>::max_digits10) << n; }

    void operator()(const String& str) const { os << '"' << str << '"'; }
    ...
    void operator()(Null null) const { os << "null"; }
  ...
  void Tokenize() {
    while (position < json_text.size()) {
      ...
      if (current_char == '-' || std::isdigit(current_char)) {
        ...
        position++;
        bool dot_flag = false;
        while (position < json_text.size() && (std::isdigit(json_text[position]) || json_text[position] == '.' && !
          dot_flag)) {
...
public:
  ...
  JsonValue parseFile(const std::string file_path) {
