...
void Scene::meshRenderSetup(Mesh *mesh)
{
	//generate VAO,VBO,EBO for the mesh
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh->getFaceRenderIndices().size() * sizeof(unsigned int), mesh->getFaceRenderIndices().data(), GL_STATIC_DRAW);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ri->edge_EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh->getEdgeRenderIndices().size() * sizeof(unsigned int), mesh->getEdgeRenderIndices().data(), GL_STATIC_DRAW);
...
bool Scene::sceneIsReady(std::string* out_error) const {
	...
	for (const auto& mesh : meshes) {
		Mesh* mesh_ptr = mesh.get();
		if (!mesh_ptr) return fail("Null mesh found in scene.");
